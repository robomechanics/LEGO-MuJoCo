...
# Modified to produce a shell instead of solid foot.
...
# ── Physical Part Calibration (Derived from real robot hardware measurements) ──
FOOT_MATERIAL_MASS_KG    = 1.062
FOOT_MATERIAL_VOLUME_MM3 = 5687625.50487
FOOT_DENSITY_KG_PER_M3   = (FOOT_MATERIAL_MASS_KG / FOOT_MATERIAL_VOLUME_MM3) * 1e9
...
# Mapping to match the desired density to the correct section of the foot during replacement
SUFFIX_TO_DENSITY = {
    "1": FRONT_BACK_DENSITY_KG_PER_M3,
    "2": MIDDLE_DENSITY_KG_PER_M3,
    "3": FRONT_BACK_DENSITY_KG_PER_M3,
}
...
def validate_ellipsoid_box_fit(X: float, Y: float, Z: float, box_x: float, box_y: float) -> None:
    ...
    raise a clear error *before* calling OpenSCAD, instead of a
...
def compute_section_bounds(box_x: float, swap_front_back: bool = False) -> dict:
    ...
    Current middle section length matches robot, but could be set to Box_x/3 if desired.
...
def compute_mesh_volume_m3(mesh_file_path: str) -> float:
    """
    Computes a mesh's enclosed volume (m^3) using MuJoCo's own mesh volume
    integration: build a throwaway one-geom model with density=1 and read
    off the resulting body mass, which numerically equals volume.
    """
    tmp_spec = mujoco.MjSpec()
    tmp_spec.add_mesh(name="_volume_probe_mesh", file=str(mesh_file_path))
    body = tmp_spec.worldbody.add_body()
    body.add_geom(
        type=mujoco.mjtGeom.mjGEOM_MESH,
        meshname="_volume_probe_mesh",
        density=1.0,
    )
    tmp_model = tmp_spec.compile()
    return float(tmp_model.body_mass[1])


def compute_section_masses(sections: dict) -> dict:
    """
    Mass (kg) of each generated section, from its mesh volume times its
    per-suffix density. Shape matches `sections`:
        {"right": {"front": kg, "middle": kg, "back": kg}, "left": {...}}
    """
    section_to_suffix = {v: k for k, v in SUFFIX_TO_SECTION.items()}
    masses = {"right": {}, "left": {}}
...
def report_actuators(model: "mujoco.MjModel") -> None:
    """
    Prints every actuator in the compiled model (name, gain/bias/dyn type,
    gear, ctrlrange, forcerange) so the <motor> definitions from the
    incoming robot.xml can be visually confirmed as unchanged after the
    foot geoms are swapped.

    This script never touches spec.actuators/spec.joints -- only geoms on
    the foot bodies are deleted/re-added -- so actuator definitions should
    be byte-for-byte preserved through spec.compile(). A plain <motor> (and
    an equivalently-configured <general>) both compile down to
    gaintype=FIXED, biastype=NONE, dyntype=NONE; any actuator that doesn't
    match that pattern is flagged below as a WARNING purely so it's easy to
    ...
    for i in range(model.nu):
        ...
        biastype_enum = mujoco.mjtBias(model.actuator_biastype[i])
        dyntype_enum = mujoco.mjtDyn(model.actuator_dyntype[i])
        gaintype, biastype, dyntype = gaintype_enum.name, biastype_enum.name, dyntype_enum.name
        gear = model.actuator_gear[i]
        ctrlrange = model.actuator_ctrlrange[i]
        forcerange = model.actuator_forcerange[i]
        is_motor_like = (
            gaintype_enum == mujoco.mjtGain.mjGAIN_FIXED
            and biastype_enum == mujoco.mjtBias.mjBIAS_NONE
...
def inject_feet_into_model(
    ...
    for side in ["right", "left"]:
        for suffix, section in SUFFIX_TO_SECTION.items():
            ...
            section_density = SUFFIX_TO_DENSITY[suffix]
            section_mass_kg = compute_mesh_volume_m3(str(new_mesh_path)) * section_density
    ...
    # Foot geoms were the only thing changed above so the original robot.xml's <motor>
    # actuators should be fully intact. Print them out to confirm.
    report_actuators(model)
